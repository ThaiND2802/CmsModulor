...
public sealed class CatalogDbContextFactory : IDesignTimeDbContextFactory<CatalogDbContext>
{
    public CatalogDbContext CreateDbContext(string[] args)
    {
        ...
        var connectionString = configuration.GetConnectionString("Default")
            ...
            ?? throw new InvalidOperationException("ConnectionStrings:Default is missing for design-time CatalogDbContext creation.");

        var optionsBuilder = new DbContextOptionsBuilder<CatalogDbContext>();
        optionsBuilder.UseDefaultDatabase(connectionString);

        return new CatalogDbContext(optionsBuilder.Options, new DesignTimeDateTimeProvider(), new DesignTimeCurrentUser());
    }
