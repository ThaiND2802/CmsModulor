...
public sealed class SaleDbContextFactory : IDesignTimeDbContextFactory<SaleDbContext>
{
    public SaleDbContext CreateDbContext(string[] args)
    {
        ...
        var connectionString = configuration.GetConnectionString("Default")
            ...
            ?? throw new InvalidOperationException("ConnectionStrings:Default is missing for design-time SaleDbContext creation.");

        var optionsBuilder = new DbContextOptionsBuilder<SaleDbContext>();
        optionsBuilder.UseDefaultDatabase(connectionString);

        return new SaleDbContext(optionsBuilder.Options, new DesignTimeDateTimeProvider(), new DesignTimeCurrentUser());
    }
